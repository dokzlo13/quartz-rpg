...
2. В игре присутствуют два обязательные части, которые задают концепцию персонажа - это [[#Классы]] и [[#Группы]]. 
    ...
    2. Группы отражают обобщенные определения черт, навыков, связей и информации, которой обладает персонаж.
        1. Группы бывают трёх типов - [[#Вид (Species)]], [[#Призвания (Vocations)]] и [[#Принадлежности (Affilations)]]. Игромеханически они дают одинаковые преимущества для конкретного персонажа, но отличаются возможностью изменения и некоторыми нюансами.
    ...
    3. Основные характеристики персонажей - "привычные" Сила, Ловкость, Телосложение, Интеллект, Мудрость, Харизма. Вместо привычных навыков (типа запугивания или атлетики) используются [[#Навыки и «тренированные броски»|тренированные броски]] основных характеристик в сочетании с [[#Группы|группами]] персонажа.
3. Сражения
...
6. Рекомендации
    1. Нечёткость формулировок даёт большой простор для творчества. Когда возникает новый вопрос по применению правил или механик, он решается групповым консенсусом с финальным вердиктом Мастера. В следующий раз механики применяются таким же образом. Это называется прецендентом.
...
Обычно это могут быть значения характеристики (STR, DEX, ...), значения Спасброска (*ST., Saving Throw*) или вашего Значения Атаки (*AV., Attack Value*). 
Например:
...
В некоторых случаях мастер может попросить добавить бонусы (+2 или другие) или штрафы (-2 или другие) к целевому значению  `target + бонус/штраф = Adjusted target` . В это случае вы должны сделать бросок ниже скорректированного значения (*Adjusted target*).
Например:
...
В системе присутсвуют критические провалы (выбрасывание 20 на кубике) и критические успехи (выбрасывание на кубике значения, равного целевому значению). 
...
Обратите внимание, что это не знаменитое правило преимущества/помехи.
"Лучший результат" обычно означает, что выбирается значение, меньше значения характеристики, однако наиболее близкое к значению целевому значению.
"Худший результат" обычно означает, что выбирается значение, больше значения характеристики, или, если оба значения кубиков меньше сложности - самое маленькое из двух.

Примеры позитивных двойных бросков:
- Персонаж с STR 14 совершает двойной положительный бросок и выбрасывает 9 и 13. Оба значения ниже значения характеристики. В этом случае лучшим значением будет 13, поскольку оно ближе к целевому значению.
- Персонаж с SV 13 совершает двойной положительный бросок и выбрасывает 5 и 19. В этом случае лучшим значением будет 5, поскольку только это значение меньше целевого значения.
...
Больше о бросках в разеле [[#Основные правила]].
...
Пример групп: *Эльфийский боец - член Тайного Братства*, является персонажем рассы (species) эльф, призвания (vocation) боец и принадлежит (affilations) к Тайному Братству. 

Вы можете создать своего эльфийского бойца в любом игровом классе, и каждый даст вам свой взгляд на концепцию этого персонажа. Традиционное правило `класс ---> концепция персонажа` в Whitehack преобразуется в `класс -×-> концепция персонажа`.
...
Например, если ваш персонаж — Мудрый (Wise) Чародей и у него есть чудо (*magick, miracle*) под названием «*Опознание*», вы не ищете конкретный текст правил, в котором подробно рассказывается, что делает эта магия. Название "Опознание" уже несёт в себе смысл того, как это чудо может работать. 
Если чудо применяется "в прямом смысле", то персонаж просто узнает об опозноваемом предмете, потратив 1 или 2 HP.
...
Но некоторым игрокам нравится пробовать множество разных вариантов, и для них, конечно, это может быть более долгий процесс. 
Если вы спешите, просто перенесите предварительно сгенерированного Мастером персонажа на свой лист персонажа. 
Если у вас есть несколько минут, выполните следующие действия:
...
Базовыми характеристиками являются числа, которые представляют силу, ловкость, телосложение, интеллект, мудрость и харизму персонажа. 
Бросьте 3d6 для каждой характеристики. Значения характеристик повышаются по мере повышения уровня персонажа, но низкие значения также могут быть полезными (см. [[#Группы]]). 
...
- ХР. (*Experience Points, Оп., Опыт*) —  Это количество очков опыта, необходимое для перехода на следующий уровень. 
...
- Ra. (*Raise, ПОВ., Повышение*) — Вы можете поднять один атрибут на 1 очко на четных уровнях.
...
Наличие соответствующей группы при выполнении [[#Навыки и «тренированные броски»|тренированных бросков ]] (*trained rolls*), означает, что вы не получаете тех же штрафов, что и персонажи без такой группы.

Каждая группа написана рядом с характеристикой на листе персонажа. Это означает, что группа имеет ещё бОльший вес (вы получаете положительный двойной бросок) при выполнении бросков задач с использованием этой конкретной характеристики — опять же, если группа имеет какое-либо отношение к броску. 

Поскольку ловкие персонажи получают положительный двойной бросок для всех бросков задач, относящихся к их группе призвания (vocation), они не отмечают его рядом с какой-либо характеристикой (см. [[#Ловкий (The Deft)|Класс: Ловкий]]).

Когда персонаж достигает нового уровня, одна группа может быть перемещена в другую характеристику.

Каждая характеристика может иметь две группы, но за каждую характеристику со значением 5 или меньше персонаж получает одну дополнительную группу принадлежности (см. [[#Принадлежности (Affilations)|Affilations]]). Если эта характеристика со временем становится выше 5, то эта группа принадлежности теряется.

В игровом мире персонаж может состоять в прочих группах. Но он не будет получать от них никакого преимущества в игромеханическом смысле, пока они не будут написаны рядом с характеристиками.

### Вид (Species)

Первый тип группы — это вид (species) персонажа. Он должен быть выбран при создании персонажа и определяется физическими и широкими культурными чертами. 
...
Распространенными группами фэнтезийных видов являются дварфы, эльфы, полурослики, гоблины и т.д. Выбор одного из них будет означать, что у персонажа есть некоторые физические и культурные черты этого вида. Среди представителей одного вида так же могут быть внутривидовые или культурные различия, дальнейший групповой выбор может углубляться в эти особенности.

Группа вид отличается от остальных. Вы отмечаете её рядом с двумя характеристиками, но их также можно использовать отрицательно по усмотрению Мастера, например, «Дварфы не созданы для быстрого плавания — сделайте двойной отрицательный бросок». 

Если только один из родителей персонажа является представителем другого вида, отличного от наиболее распространённого, отметьте группу видов рядом с одним атрибутом. 
...
И наоборот, [[#Мудрый (The Wise)]] рыцарь будет физически слабее своих коллег-рыцарей, но все же будет хорошим тактиком с магией, которая поддержит его в бою. 

Призвания — это не просто занятия. Персонаж может иметь только одно призвание, но может заменить или развить его в рамках своей карьеры, если есть возможность в игре.
...
В некоторых кампаниях могут быть группы принадлежности, такие как Добро, Зло, Закон и т. д. Они обозначают моральную настройку на богов и другие силы. Персонажи без таких групп просто неоднозначны и сложны в своих нравах.
...
Каждому чуду дается описательная или вызывающая воспоминания **формулировка**, например «Телекинез» или «Патрок, Демон Перехода!». Мудрые используют мистические силы для нарушения естественного порядка вещей. Обратной стороной такого вмешательства всегда является необходимость траты собственных жизненных сил, выраженная через трату HP персонажа.
...
Игрок может понизить стоимость с помощью недостатков или использования ценных ингредиентов. Затраты увеличиваются по мере того, как желаемые эффекты удаляются от формулировки (wording) заклинания, призвания (vocation) персонажа и того, чего можно достичь без магии. Вы можете намеренно выбрать точные формулировки для своих чудес для снижения затрат или расплывчатые формулировки для универсальности. (см. [[#Изменения стоимости чудес|таблицу корректировки стомиости]])

Существуют рекомендации по стоимости, основанные на **порядках вмешательства (*порядок вмешательста, магнитуда, magnitude*)** (см. [[#Порядки вмешательства|таблицу порядков вмешательства]]). Но затраты, как правило, немного меняются в зависимости от времени и места. Мудрый может один раз в день сэкономить HP и потратить 10 минут на обнаружение и использование локальной концентрации энергии, удешевляя одиночный эффект определенного типа на однин порядок вмешательства.
...
Мудрые не могут восполнять утраченные HP при помощи заклинаний, свитков или зелий (хотя они могут им понадобиться для других целей, например очищения от проклятия или лечения болезней). 
При этом, Мудрые по-прежнему могут получить первую помощь, предотвращающую смерть в бою (см. [[#Травмы и исцеление|]]). ^[Моё расширение стандартных правил]

#### Механические особенности

Нося щиты или доспехи тяжелее кожанных, Мудрые получают штраф +2 ко всем затратам HP (до любого увеличения или удвоения). У них есть -2 AV при использовании двуручного оружия, за исключением случаев, когда оружие находится в ячейке, как указано [[#Создание и использование магических предметов|ниже]]. Они получают +2 ST против магических и влияющих на разум способностей.
...
Любой персонаж может использовать свитки (стр. 78), но в отличии от других, Мудрые могут использовать свитки иначе.

Как только магия достаточно глубоко укоренилась в символах свитка, Мудрый может сохранить эффект в слот при успешной тренированной проверке интеллекта. Уровень Мудрого должен превышать стоимость свитка в HP, а свитку должно быть не менее десяти лет. 
...
В ячейке 3-го уровня вместо чудес может находиться магический предмет — черный посох, говорящий меч и т. д. Это увеличивает максимальные и текущие HP персонажа на величину, равную его уровню. При выборе все предыдущие формулировки (активная и неактивная), привязанные к слоту, разрушаются. Слот можно освободить для другого предмета, или если предыдущий сломался, что снижает как максимальное, так и текущее значение HP.
...
|  2  | Стандартный (Standart)      | Нечто простое за гранью возможного без магии    | Дыхание под водой, малый Телекинез, Невидмость, Укрепление брони                             |
...
|               +               | Увеличенная область действия для массового заклинания | "Замлятрясение" действует в увеличенной области                                                                                             |
...
|               -               | Проверка характеристики для персонажа                 | Персонаж, произносщий "Очарование" делает проверку характеристики CHA (Харизма). В случае провала заклинание отменяется                     |
...
Мастеру рекомендуется допускать незначительные корректировки выбранной вами формулировки до тех пор, пока персонаж не достигнет следующего уровня. Это делает первоначальный выбор проще и часто немного быстрее. Но после этого формулировка не может быть изменена, по крайней мере, до 10-го уровня.
Это может показаться суровым, но суть чудес не в том, что ваш персонаж должен быть в состоянии делать все, что ему заблагорассудится. У вашего персонажа есть черты и индивидуальность, и каждая чудесная формулировка является частью или продолжением того, кто он есть. С игровой точки зрения также важно, что возможности Мудрого ограничены «триангуляцией» формулировок, призвания и действия, так чтобы решение внутриигровых задач с помощью магии стало интереснее. 
...
Ловкий должен выбрать призвание (vocation) на уровне 1. При правильном снаряжении и при броске задачи или атаки, соответствующих его призванию, они всегда используют положительный двойной бросок. 

По этой причине ловкие персонажи не отмечают группу призвания рядом с конкретным атрибутом. Кроме того, когда они имеют боевое преимущество (см. [[#Боевое преимущество (Combat Advantage)|Боевое Преимущество]]), ловкие персонажи могут поменять преимущество  двойного положительного броска, полученное от своего класса, на двойной урон, если их призвание соответствует ситуации. Например, странствующий торговец может сделать это, защищая свой фургон с грузом, а плут, нападая из тени.

По мере повышения уровня, Ловкие получают **слоты (Sl.)**, каждый из которых они навсегда связывают с двумя **настройками (attunement)**. Подобная настройка должна быть **учителем**, **предметом**, **прирученным животным** или **местом**. Например:
...
- Поистине невозможные действия остаются невозможными.
...
У Ловких -2 AV с двуручным оружием ближнего боя, не имеющим настройки, но они получает +1 к урону и AC от одноручного оружия, если у них боевое призвание (vocation). 
Персонажи с призваниями, не ориентированными на боевые действия, могут сделать спасбросок, чтобы при успехе превратить успешный бросок задачи в критический успех, один раз за день *(в оригинале - за игровую сессию)*.
...
Они получают ту же самую единственную базовую атаку в раунд, что и другие классы, но всякий раз, когда они добивают врага, они могут использовать импульс этого действия, чтобы атаковать другого соседнего врага. Это требует отдельного броска атаки, но не учитывается при подсчете количества бесплатных атак (см. ниже).

#### Заимствование навыков

Сильные персонажи могут получить *ключевые слова* от врагов, если они нанесут смертельный удар в памятной битве. Ключевые слова могут использоваться для трюков, специальных атак или способностей в одной из следующих категорий, определяемых во время присваивания ключевого слова:

- **Запоминающийся враг**. Если тип врага и ключевое слово относятся к действию, Сильный может получить +2 на один раунд к AC, AV, ST, HP, аттрибуту, урону, лечению, MV, инициативе. Например, боевой клич, похожий на банши, может дать +2 к инициативе.
- **Субстанция**. Сильный извлекает из трупа редкое и сильнодействующее вещество (яд, кислоту, что-то легковоспламеняющееся и т. д.).
- **Сверхъестественное**. Акт убийства передает **ненасильственную** сверхъестественную способность, которая есть у монстра (она должна работать так же), например, способность вирма "Чувство Золота".

Сильный может иметь только одно ключевое слово в любой момент времени, но его можно обменивать при убийстве новых монстров (неиспользованные вещества в этом случае считаются испорченными). Каждое ключевое слово можно использовать количество раз, равное уровню персонажа. Ключевое слово не занимает слот, но если это вещество, ему может понадобиться место в инвентаре персонажа, где оно будет учитываться.
...
У них есть бонусы СЛ против яда и смерти (+1) и специальные атаки ближнего боя (+4).

#### Боевые приёмы

Сильные могут использовать любой особый вариант боя (стр. 73), но в каждом из их слотов также есть один более сильный вариант. По мере своего роста в уровне, Сильные выбирают из списка ниже. Данные навыки нельзя заменить. Если не указано иное, эффекты длятся один раунд и могут быть активированы любое количество раз.
...
3. Сражаясь с крупными противниками, Сильный может одним действием зацепиться за него и даже взобраться на него при успешном броске ловкости, получая двойное боевое превосходство (+4 к AV и урону) в следующем и любом последующем раунде до тех пор, пока он на противнике. По усмотрению Мастера могут потребоваться новые броски ловкости, но они не требуют отдельных действий.
...
Если игрок не выбрал группу, которая делает его неуместным (например, «варвар»), все персонажи считаются грамотными. 
...
2. Бонусные языки от высокого интеллекта. Обычно бонусными языками являются драконий, гоблинский и один или несколько мертвых языков («высокий имперский», «ктонианский» и т. д.), которые можно найти в старых свитках и книгах. В обоих случаях рефери может решить, что язык, приобретенный после уровня 1, требует времени на изучение.
...
Бросьте 3d6 и умножьте результат на 10. Это количество золотых монет (gp), которые ваш персонаж получает для покупки начального снаряжения. Один золотой стоит 10 серебряных (sp) или 100 медных (cp). Ваш игровой мир может использовать другую монету, использовать другие курсы или иметь серебряный, а не золотой стандарт. Списки снаряжения не могут быть исчерпывающими — дополняйте приведенные ниже по своему усмотрению.
...
Монеты, драгоценные камни и очень маленькие предметы не имеют размера — 100 из них считаются одним обычным предметом. Используйте здравый смысл, чтобы решить, что есть что. Оружие имеет обозначение обычное/тяжелое/второстепенное.
...
Чудо "идентификации" делает то же самое, но мгновенно.

#### Магические зелья и порошки

Зелья можно выпить, что не требует навыков, или бросить в кого-то, что представляет собой бросок [[#Дальнобойная атака|дальнобойной атаки]] или атаки по области. То же самое касается атак порошками. Раздавить флакон на расстоянии до пяти футов — это автоматический успех. Известные вещества можно тщательно отобрать и распознать за 10 минут.
...
В магических свитках уже "встроено" всё необходимое — эффект, его параметры и стоимость в HP. 
Чтобы использовать свиток, вы должны знать язык и преуспеть в тренированном броске интеллекта, со штрафом, равным стоимости эффекта чуда свитка в HP. 
...
Однако многие свитки написаны на древних версиях языков, которые знают персонажи, что уменьшает штраф на усмотрение Мастера.

Подходящими группами для подготовленного броска могут быть, например, призвания (vocation) Писца, Волшебника, Монаха или Ученого, принадлежность (affilation) к Ордену Ттиллов или Культу Ведьм или даже к расам (species) Грибных Людей или Ур-Эльфов. 
...
Например, если ваш персонаж заходит в тупик и начинает искать потайную дверь, о которой говорит его карта, решение состоит не в том, чтобы просто бросить бросок, чтобы увидеть, найдет ли он что-нибудь. Вместо этого скажите Мастеру, куда смотрит персонаж, и если это правильное место, он найдёт дверь. Или, если группа окажется около крутого уступа, на который нужно будет взобраться, вы и другие игроки должны описать, как персонажи это делают — инструменты, меры безопасности и т. д., — чтобы Мастер мог позволить этому повлиять на персонажей и шансы в бросках кубика.

Это позволяет вам добиться успеха в игре. Если вы просто бросаете кости, ни одно из ваших умений (или их отсутствие) не будет иметь значения, и скорее всего приведёт к поражению. Опять же, вы должны быть в состоянии сыграть персонажа, который хорош в том, в чем вы не так сильны. Например, если вы любите загадки, но никогда не можете их разгадать, бросок кубика на интеллект вашего персонажа Мудрого Исследователя может дать вам надежную подсказку.
...
XP за выполнение квестов представляет собой опыт, который персонажи получают за достижение определенных целей, а не просто за выживание. Обнаружение силы, стоящей за вторжением кобольдов, может быть целью квеста. Свергнуть королеву и посадить на ее трон кого-то другого — другой квест. В приключении часто есть несколько небольших квестов, которые нужно выполнить, а также основной квест. В типичном низкоуровневом приключении часть квеста составляет примерно половину от общего количества получаемого опыта. По мере того, как персонажи повышают уровень и выполняют более сложные миссии, эта квота может увеличиваться. Иногда вы терпите неудачу и вообще не получаете квестовый опыт. В других случаях квестовый опыт может быть всем, что вы получите.
...
В противном случае это не так (некоторые спасброски также могут иметь особые эффекты в случае «критического провала» или требовать соответствующее «качество» броска,). 
Любой устойчивый или уязвимый к определенной опасности использует положительные или отрицательные двойные броски соответственно.

### Броски задач, навыки и качество броска

Когда персонаж преднамеренно берется за рискованную и сложную задачу, Мастер говорит игроку бросить d20. 
**Если результат равен или ниже соответствующей характеристики, задача выполнена успешно. В противном случае это не так.** 
...
#### Сложность, помощь и экстремальные значения
...
В этом случае Мастер может попросить игрока добавить (бонус) +2 или вычесть (штраф) -2 или более к используемой характеристики.
Примеры:
...
Иногда эта или какая-либо другая модификация приводит к экстремальным значениям характеристик. Например, штраф от проверки  может сделать целевое значение меньше 1 или больше 20. В этих случаях:
- При целевом значении меньше 1 он будет автоматически провален (вы можете не делать бросок);
- При целевом значение 20 и выше бросок 19 считается критическим успехом, а 20 считается обычным провалом.

У вас может возникнуть соблазн использовать механизм AC (КД) или двойные броски для сложности. Первое решение работает только для более сложных действий, а не для более простых. Второе решение не различает сложное/сложнее или легкое/проще. Это делает навыки игрока менее важными. В обоих случаях у вас возникают проблемы, когда вам нужно совместить сложность с AC и группами. Использование модификаций сложности решает эту проблему.

#### Навыки и «тренированные броски»

Характеристики не говорят всего о способности персонажа выполнить задачу. Вид, призвание и даже принадлежности персонажа указывают на то, какими предрасположенностями, опытом и особыми знаниями он обладает. 
Когда группа написана рядом с характеристикой, персонаж может сделать положительные двойные броски для выбранной характерисики — если группа кажется релевантной для задачи. 
Например, Мудрый персонаж с призванием «священник», написанным рядом с харизмой, получит положительную двойную проверку при попытке успокоить разъяренную толпу.
...
Мудрый персонаж, у которого «священник» написано рядом с интеллектом, тоже не делает обычную проверку, потому что это не соответствующий атрибут (разъяренная толпа обычно не слушает голос разума).
...
- Если эта группа написана рядом с соответствующим атрибутом, она получает положительный двойной бросок. 
- Но если персонаж не имеет ни одной подходящей группы в предыдущих двух категориях, задание требует отрицательного двойного броска.

#### Пары (Pairs)

Получение пары в успешном положительном двойном броске означает, что персонажу удалось получить какую-то дополнительную выгоду, кроме успеха. 
Получение пары в неудачном отрицательном двойном броске означает получение какого-то дополнительного пагубного эффекта, кроме провала.
...
- Успешная положительная пара, 
- Критический успех
- Успех, 
- Неудача
- Критический провал (fumble), 
- Неудачная отрицательная пара. 

Для более детального сравнения Мастер может позволить более высокому броску в той-же категории быть выигрышным.
...
**Чтобы попасть, бросок должен быть успешным и иметь качество (*quality*) строго выше класса брони противника.** 
Обратное правдиво и для вас. Чтобы попасть по вам атакой, противник должен сделать такой-же бросок против .

Например, кто-то с AV 14 и мечом +1, сражающийся с гоблином AC 3, попадет на 4–15. 

Модификаторы для снаряжения могут быть учтены на листе персонажа, что делает операцию еще проще. Применяются все правила для бросков задач, включая критические успехи и провалы и возможность использовать лишние очки (например, для модифицированного AV выше 20) для повышения качества. 
Нащупывание и криты также имеют дополнительные правила, которые представлены на следующем развороте. Ловкие персонажи иногда используют в атаках два кубика: удачная пара означает попадание и боевое превосходство в следующем раунде.

Критические успехи и провалы требуют бросков по таблицам ниже. 
Если критический удар пришёлся по существу в шлеме, вы должны перебросить результаты 16+ один раз.

Вы можете сделать спасбросок от критического провала, чтобы вместо этого просто провалиться. 
...
Обладая боевым преимуществом, [[#Ловкий (The Deft)|Ловкие]] персонажи всегда используют двойной положительный бросок, и могут вместо него нанести двойной урон, когда они попадают по противнику в соответствии с их призванием.

Если боевое преимущество, получаемое [[#Ловкий (The Deft)|Ловкими]] персонажами, связано с их призванием (vocation), они используют двойной положительный бросок для атаки, и могут заменить его на на двойной урон. ^[Deft characters can switch their positive double roll for double damage when they strike with combat advantage in a way that relates to their vocation]
...
Чтобы выйти из боя, вы должны для этого потратить действие, чтобы отойти назад в осторожном темпе (с. 65н), по-прежнему лицом к своим противникам, иначе противники могут наносить свободные атаки.
...
| Действие  | Эффект                                                                                                                                                                                                                                                                                          |
| --------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Рывок     | Рывок на 15 футов по прямой к противнику с боевым превосходством. Вы получаете -3 AC до своего следующего хода.                                                                                                                                                                                 |
...
\*\* Обратите внимание, что этот вариант имеет математический смысл только для (модифицированного) AV 20+ или против очень высокого ST.
...
Недостатком может быть например, пара потерянных пальцев, сломанная нога или обезображивающий шрам, и Мастер может использовать это, чтобы потребовать отрицательного двойного броска или свести на нет положительное преимущество двойного броска группы персонажа для конкретной задачи.
...
### Глоссарий
- AC. (*Armour Class, КД., Класс Доспеха*) — Это значение, которое используется для расчёта попадания по вам атаками. 
- 
